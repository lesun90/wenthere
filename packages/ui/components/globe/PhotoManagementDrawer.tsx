...
export function PhotoManagementDrawer({
  ...
  useEffect(() => {
    ...
    const photo = findPhoto(editingPhotoId, profileIndex, unplacedPhotos)
    if (photo) setDraft(draftFromPhoto(photo, profileIndex))
  }, [editingPhotoId, profileIndex, unplacedPhotos])

  useEffect(() => {
    if (!pendingEditPhotoId) return
    setEditingPhotoId(pendingEditPhotoId)
    onPendingEditPhotoHandled?.()
  ...
  function handleSave() {
    if (!editingPhotoId) return
    setActionError(null)
    Promise.resolve(onEditPhoto?.(editingPhotoId, draft))
      .then(() => setEditingPhotoId(null))
      .catch(error => setActionError(error instanceof Error ? error.message : 'Could not save photo changes.'))
  }

  // ── derived ──
  const isEmpty = profileIndex.stats.photoCount === 0 && unplacedPhotos.length === 0
  const isFiltered = q.length > 0
  const hasResults = filteredEntries.length > 0
  const editingPhoto = editingPhotoId ? findPhoto(editingPhotoId, profileIndex, unplacedPhotos) : null

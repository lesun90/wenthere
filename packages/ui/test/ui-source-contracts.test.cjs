...
assert.match(galleryPanel, /const countryPhotos = profileIndex\.photosByCountryCode\[countryCode\] \?\? \[\]/)
assert.match(galleryPanel, /countryPhotos\.length === 1/)
assert.match(galleryPanel, /photos\.length > 0 && \(\s*<div\s*ref=\{thumbsRef\}/)

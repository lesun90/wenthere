...
// UI_DATA_DIR is set by next.config.ts (apps/web) to point to packages/ui/data.
// Falls back to process.cwd()/data for CJS check scripts run from packages/ui/.
const SUBDIVISION_DIR = join(
  process.env.UI_DATA_DIR ?? join(process.cwd(), 'data'),
  'geo', 'subdivisions',
)
